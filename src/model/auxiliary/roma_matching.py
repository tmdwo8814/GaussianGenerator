...
class RoMaMatcher:
    ...
    def initialize(self, device: torch.device):
        ...
        if self._model is not None:
            ...
            return
        if device.type != "cuda":
            raise RuntimeError("The RoMaV2 auxiliary training path requires CUDA")
        try:
            from romav2 import RoMaV2
        except ImportError as error:
            raise ImportError(
                "RoMaV2 is not installed. See docs/romav2_aux.md and "
    ...
    def match(self, images: Tensor) -> Correspondences:
        ...
        # can safely be saved by autograd in the later photometric computation.
