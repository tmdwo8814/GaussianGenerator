...
# ---------------------------------------------------------------------------
# Stats Auxiliary Head
# ---------------------------------------------------------------------------
class StatsAuxHead(nn.Module):
    """
    Lightweight auxiliary head on the shared DPT feature (256ch).
    Predicts per-pixel alpha-blending statistics:
        ch0: T_pred        (transmittance)
        ch1: sigma_D2_pred (depth variance)

    Input : (B, 256, H, W)
    Output: (B,   2, H, W)  in [0,1] via Sigmoid
    """
    def __init__(self, feat_dim: int = 256):
        super().__init__()
        self.net = nn.Sequential(
            nn.Conv2d(feat_dim, 64, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(64),
    ...
    def forward(self, dpt_feat: torch.Tensor) -> torch.Tensor:
        return self.net(dpt_feat)


# ---------------------------------------------------------------------------
# Modified DPTOutputAdapter_fix
# ---------------------------------------------------------------------------
class DPTOutputAdapter_fix(DPTOutputAdapter):
    """
    Adapted from croco's DPTOutputAdapter for NoPoSplat.
    Changes vs. original:
      - Removes duplicated act_N_postprocess weights.
      - Adds StatsAuxHead branch when head_type == 'gs_params'.
      - forward() returns (gs_out, stats_pred) for gs_params head,
        plain tensor otherwise.
    """
    ...
    def init(self, dim_tokens_enc=768):
        ...
        # Stats auxiliary head — only for the GS params DPT head
        if self.head_type == 'gs_params':
            self.stats_aux_head = StatsAuxHead(feat_dim=self.feature_dim)

    def forward(self, encoder_tokens: List[torch.Tensor], depths, imgs,
                image_size=None, conf=None):
        assert self.dim_tokens_enc is not None, \
            'Need to call init(dim_tokens_enc) first'
        ...
        layers = [rearrange(l, 'b (nh nw) c -> b c nh nw', nh=N_H, nw=N_W)
                  for l in layers]
        ...
        dpt_feat = path_1 + direct_img_feat    # (B, 256, H, W) — shared feature

        # Branch: stats auxiliary head
        if self.head_type == 'gs_params':
            stats_pred = self.stats_aux_head(dpt_feat)   # (B, 2, H, W)
            gs_out = self.head(dpt_feat)
            return gs_out, stats_pred

        return self.head(dpt_feat)
...
class PixelwiseTaskWithDPT(nn.Module):
    """DPT module wrapper used by head_factory."""
    ...
    def forward(self, x, depths, imgs, img_info, conf=None):
        result = self.dpt(x, depths, imgs,
                          image_size=(img_info[0], img_info[1]), conf=conf)

        # gs_params head returns (gs_out, stats_pred)
        if isinstance(result, tuple):
            gs_out, stats_pred = result
            if self.postprocess:
                gs_out = self.postprocess(gs_out, self.depth_mode, self.conf_mode)
            return gs_out, stats_pred
