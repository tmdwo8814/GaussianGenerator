...
@torch.no_grad()
def build_knn(points: Tensor, k: int = 16, workers: int = 1) -> Tensor:
    ...
    Only this discrete search uses detached CPU coordinates. Callers must
    ...
    if k == 1:
        return torch.from_numpy(own.copy()).to(points.device)
